...
public class PatternServerAdminTest
{
	...
	{
		Assert.assertTrue(xTeam.getInstance().getCommandManager().getUsage("serveradmin_" + baseCmd).replaceAll("Page", "1").replaceAll("[\\[\\]\\{\\}]", "").matches("/team " + xTeam.getInstance().getCommandManager().getPattern("serveradmin_" + baseCmd)));
	}
}
